...
trait ProductDelete_Process
{
  ...
  {
    try {
      Collection::whereIn('id', $collectionid)->delete();
    } catch (Exception $e) {
      throw new Exception($e->getMessage());
    }
  }
  public function deleteCollectionProduct($collectionid)
  {
    try {
      $collectionId = CollectionProduct::whereIn('collection_id', $collectionid)->pluck('id')->toArray();
      CollectionProduct::whereIn('id', $collectionId)->delete();
    } catch (Exception $e) {
      throw new Exception($e->getMessage());
    }
  }
