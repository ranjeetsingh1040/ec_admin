...
class ProductCategoryController extends Controller
{
    ...
    public function getProductCategory(Request $request, $sellerid) //display product list
    {
      $show=(int)$request->show;
      
      if($show!=0){
        $product=Product::with('productcategory','stores')->where('user_id',$sellerid)->orderBy('id', 'DESC')->paginate($show);
                               
      }
      else{
        $product=Product::with('productcategory','stores')->where('user_id',$sellerid)->orderBy('id', 'DESC')->paginate(15);
