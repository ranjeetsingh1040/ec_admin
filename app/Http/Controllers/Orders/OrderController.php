...
class OrderController extends Controller
{
  ...
  public function store(OrderStoreRequest $request, Cart $cart)
  {
    ...
    try {
      ...
      $shippingAddress = Address::with('state:name,id','city:name,id','country:name,id')->where('id', $request->address_id)->get()->first();
      // $billingAddress = json_encode($address);
      // return response()->json(['success' => false, 'message' => $shippingAddress], 200);
      
      $order = $this->createOrder($request, $cart, $shippingAddress);
  ...
  protected function createOrder(Request $request, Cart $cart, $shippingAddress)
  {
    ...
    try {
      ...
      foreach ($request->order_item_id as $key => $value) {
        ...
        if ($orderproduct->status != 'completed') {
          ...
          $this->createCreditTransaction(
            ...
            $status,
            $action,
            $request->accounting_code,
            $request->$comments,
            NULL,
            NULL,
            $orderproduct->id,
            get_class($orderproduct),
            $array,
            $id
          );
      ...
      if ($this->getOrderCompletionStatus($request->order_id) == true) {
        ...
        if ($order->giftorder_id != null) {
          ...
          $this->createDebitTransaction(
            $gift_order->orderitem->seller_id,
            $gift_order->amount,
            $status,
            $action,
            $request->accounting_code,
