...
class StoreController extends Controller
{
  ...
  public function store(StoreRequest $request)
  {
    $res = [];
    try {
      $imagelogo = $request->file('storelogo');
      $namelogo = str_random(10) . '_' . time() . '-logo' . '.' . $imagelogo->getClientOriginalExtension();
      $imgpathlogo = $imagelogo->getRealPath();
      $image_resizelogo = Image::make($imgpathlogo);
      $image_resizelogo->resize(300, 300);
      $image_resizelogo->save(public_path('uploads/storeimage/' . $namelogo));
      ...
      $image_resize->save(public_path('uploads/storeimage/' . $name));

      $imagepathlogo = 'uploads/storeimage/' . $namelogo;
      $imagepathcover = 'uploads/storeimage/' . $name;
  ...
  public function update(EditStoreRequest $request, $id)
  {
    ...
    try {
      ...
      if (is_null($store)) {
        ...
        //   $store->storeimage = $imagepathcover;
        // }
        if ($request->storelogo != $store->storelogo) {
        ...
        if ($request->storeimage != $store->storeimage) {
          File::delete(public_path($store->storeimage));
          $image = $request->file('storeimage');
          $name = str_random(10) . '_' . time() . '.' . $image->getClientOriginalExtension();
          $imgpath = $image->getRealPath();
          $image_resize = Image::make($imgpath);
