...
class StoreBuyerController extends Controller
{
   public function searchPrice(Request $request, $minval, $maxval)
   {
      ...
      try {
         ...
         $products = Product::with([
            'variations.stock', 'categories',
            'rating', 'variations.type', 'users'
         ])->where('status', 1);
   ...
   public function getstoreproduct($slug)
   {
      ...
      $products = Product::with(['variations.stock', 'categories', 'rating', 'variations.type', 'users'])->where('status', 1)->where('store_id', $store->id)->withScopes($this->scopes());
      $products = $products->paginate(10);
   ...
   public function ratingFilter(Request $request)
   {
      ...
      $products = Product::with([
         'variations.stock', 'categories', 'users', 'rating', 'variations.type'
      ])->where('status', 1)->where('store_id', $store->id)->where('ratings', '>=', $request->rating)->paginate(10);
   ...
   public function brandFilter(Request $request)
   {
      ...
      $products = Product::with([
         'variations.stock', 'categories', 'users', 'rating', 'variations.type'
      ])->where('status', 1)->where('store_id', $store->id)->where('brand', $request->brand)->paginate(10);
