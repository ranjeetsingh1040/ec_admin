...
class LoginController extends Controller
{
    ...
    public function actionseller(LoginRequest $request)
    {
        ...
        if ($user->status == 0) {
            return response()->json(['errors' => ['email' => ['Your Account is deactivated.']]], 422);
        }

        if (empty(auth()->user()->email_verified_at)) {
            return response()->json(['errors' => ['email' => ['You have not verified your email address, please check your email']]], 422);
        }
        ...
        if ($request->user()->usergroup_id == 3) {
            $this->lastUserActivity(Auth::user());
