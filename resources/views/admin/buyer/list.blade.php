...
@include('admin.buyer.filter')
		<div class="seller_table bg-white">
		<div class="my-5 overflow-y-auto">
			<table class="w-full">
				<thead>
					<tr>
						<th>{{trans('buyerlist.name')}}</th>
						<th>{{trans('buyerlist.orders')}}</th>
						<th>{{trans('buyerlist.reviews')}}</th>
						<th>{{trans('buyerlist.address')}}</th>
						<th>{{trans('buyerlist.actions')}}</th>
					</tr>
				</thead>
		...
		@foreach($buyers as $buyer)
 		...
 		<img src="{{url($buyer->image)}}" class="w-8 h-8 rounded-full">
			...
			<a href="{{url('/admin/buyer/'.$buyer->id.'/details')}}">
				<div class="text-gray-700 mx-2 leading-relaxed">
					<p class="text-xs text-orange font-semibold">{{$buyer->name}}</p>
					<p class="text-xs">{{$buyer->email}}</p>
				</div>
			</a>
				</div>
			...
			<td class="px-3 py-3">
				<p class="text-sm">{{$buyer->orders->count()}}</p>
			</td>
			<td class="px-3 py-3">
				<p class="text-sm">{{$buyer->ratingReview->count()}}</p>
			</td>
			
			<td class="px-3 py-3">
				<p class="text-sm whitespace-nowrap lg:whitespace-normal md:whitespace-normal">
	...
	<g>
		...
		@endforeach
		  ...
		  <div class="custom-paginate py-2">
			{{ $buyers->appends(['search' =>request()->get('search')])->links() }}
			  @if(!count($buyers)>0)
...
function change()
{
	...
	window.location.href="{{url('/admin/buyer?sort_by=')}}"+$sort_by+"&paginate="+$paginate+"&search="+$search;
}
