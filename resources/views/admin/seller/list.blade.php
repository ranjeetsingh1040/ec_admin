                ...
                <tbody>
                    @foreach ($sellers as $seller)
                        <tr class="seller_list rounded  bg-white">
                            ...
                            <td class="px-3 py-3"><p class="text-sm">{{ $seller->mystores->count() }}</p></td>
                            <td class="px-3 py-3"><p class="text-sm">{{ $seller->orderItem->count() }} </p></td>
                            <td class="px-3 py-3"><p class="text-sm">
                            ...
                            <td class="px-3 py-3"><ul class="list-reset flex items-center action_icon"><li>
                                        <a href="{{ url('/admin/seller/' . $seller->id . '/details') }}">
                            ...
                            <td>@if ($seller->sellerprofile->status=="approved")
                                {{"Approved"}}
                                @elseif(($seller->sellerprofile->status=="not_approved"))
                                {{"Not Approved"}}
                                @else
                                {{"Registration Pending"}}
                                {{-- {{ $seller->sellerprofile->status }}</td> --}}
                                @endif
                        </tr>
                        <!-- ****** -->
                    @endforeach
...
@push('scripts')
    ...
    <script>
        function change() {
            ...
            window.location.href = "{{ url('/admin/seller?sort_by=') }}" + $sort_by + "&paginate=" + $paginate +
                "&search=" + $search;
        }
    </script>
