            ...
            <table class="w-full">
                <thead>
                    <tr>
                        ...
                        <th width="10%">{{ trans('details.image') }}</th>
                        <th width="15%">{{ trans('details.name') }}</th>
                        <th width="10%">{{ trans('details.date') }}</th>
                        <th width="10%">{{ trans('details.ratings') }}</th>
                        <th width="10%">{{ trans('details.status') }}</th>
                        <th width="10%">{{ trans('details.edit') }}</th>
                    ...
                    @foreach ($ratingReviews as $ratingReview)
                        <tr>
                            ...
                            <td class="px-2 py-1"><img
                                    src="{{ url($ratingReview->product->productgallery[0]->thumbnailimage) }}"
                                    class="w-16 h-10"></td>
                            <td class="px-3 py-3">
                            ...
                            <td class="px-3 py-3">
                                <p class="text-xs">
                                    <!-- {{ date('d-m-Y', strtotime($ratingReview->created_at)) }} -->
                                    {{ $ratingReview->created_at }}
                            </td>
                            <td class="px-3 py-3">
                                <ul class="list-reset flex items-center">
                                    @for ($i = 1; $i <= 5; $i++)
                                        @if ($i <= $ratingReview->rating)
                                            <li class="mr-1"><img src="{{ url('/images/icons_1/star_fill.svg') }}"
                                                    class="w-3 h-3"></li>
                                        @else
                                            <li class="mr-1"><img src="{{ url('/images/icons_1/star_line.svg') }}"
                                                    class="w-3 h-3"></li>
                                        @endif
                                    @endfor
                                </ul>
                            ...
                            <td class="px-3 py-3">
                                <form method="post" action="{{ url('/admin/reviews/edit') }}" accept-charset="UTF-8">
                                    ...
                                    @if ($ratingReview->status == 'not_approved')
                                        ...
                                        <input type="submit"
                                            class="bg_green rounded py-1 px-2 text-xs text-white cursor-pointer"
                                            name="status" value="Approved">
                                    @else
                                        <input type="hidden" name="id" value="{{ $ratingReview->id }}">
                                        <input type="submit"
                                            class="bg_red rounded px-2  text-xs py-1 text-white cursor-pointer"
                            ...
                            <td class="px-3 py-3">
                                <ul class="list-reset flex items-center action_icon">
                                    ...
                                    <li class="mx-1">
                                        <a class="del"
                                            rel="{{ url('/admin/review/' . $ratingReview->id . '/delete') }}">
                                            <svg class="fill-current w-4 h-4" xmlns="http://www.w3.org/2000/svg"
                                                height="512px" viewBox="-47 0 512 512" width="512px">
                                                <g>
                                                    <path
                                                        ...
                                                        data-original="#000000" class="active-path" fill="" />
                                                    <path
...
@push('scripts')
    <script>
        function change() {
            ...
            window.location.href = "{{ url('/admin/reviews?sort_by=') }}" + $sort_by + "&paginate=" + $paginate +
                "&search=" + $search;
        }
    </script>
